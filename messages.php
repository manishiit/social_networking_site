...
<?php 
	if(isset($_GET['u_id'])) { 
		...
		$user_to_msg= $row_user['user_id']; $user_to_name = $row_user ['user_name'];
	}
	...
	<div class="col-sm-3" id="select_user">
		<?php
			...
			while($row_user= mysqli_fetch_array($run_user)){
				$user_id = $row_user['user_id']; $user_name = $row_user['user_name']; 
	...
	<div class="col-sm-6">
		<div class="load_msg" id="scroll_messages">
			<?php 
				...
				while($row_msg = mysqli_fetch_array($run_msg)) {
					...
					$user_from= $row_msg['user_from']; $msg_body = $row_msg['msg_body'];
					$msg_date= $row_msg['date'];
					?>

					<div id="loaded_msg">
						<p><?php if($user_to== $user_to_msg AND $user_from ==
						$user_from_msg) {echo "<div class='message' id='blue' data-toggle = 'tooltip' title='$msg_date'>$msg_body</div><br><br><br>";} 
						else if($user_from== $user_to_msg AND $user_to==
						$user_from_msg) {echo" <div class='message' id='green' data-toggle='tooltip' title='$msg_date'>$msg_body</div><br><br><br> ";}?></p>
					</div>
			...
			if(isset($_GET['u_id'])){    
				...
				echo'
					<form action="" method="POST">
						...
						<input type="submit" name="send_msg" id="btn-msg" value
						= "Send">
					</form><br><br>
				';	
		...
		<?php 
			if(isset($_POST['send_msg'])) {
				...
				}else if(strlen($msg) > 37){ echo "<h4 style='color:red; text-align: center;'    >Message is too long!
				    Use only 37 characters</h4>";
				}else{
	...
	<div class="col-sm-3">
		<?php 
			if(isset($_GET['u_id'])) { 
				...
				$get_user= "select * from users where user_id='$get_id'";
				$run_user= mysqli_query($con, $get_user); $row = mysqli_fetch_array($run_user);
				
				$user_id = $row['user_id'];
				$user_name = $row['user_name'];
				$f_name = $row['f_name']; 
				$l_name = $row['l_name'];
				$describe_user= $row['describe_user']; $user_country = $row['user_country'];
				$user_image = $row['user_image']; $register_date= $row['user_reg_date']; $gender = $row['user_gender'];
			}
			if($get_id == "new"){
					...
					<div class='row'>
						...
						<div style='background-color: #e6e6e6;' class='col-sm-9'>
							...
							<ul class='list-group'>
								...
								<li class='list-group-item' title='Country'> $user_country
								</li>
